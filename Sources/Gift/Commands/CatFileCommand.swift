...
struct CatFileCommand: CommandProtocol {
    ...
    func run(_ options: CatFileCommand.Options) -> Result<(), CatFileCommand.ClientError> {
        guard let type = GitObjectType(rawValue: options.type) else {
        ...
        do {
            object = try repository.readObject(sha: repository.findObject(name: options.object))
        } catch let error {
            return .failure(CommandantError.usageError(description: error.localizedDescription))
        }
...
struct CatFileOptions: OptionsProtocol {
    ...
    public static func evaluate(_ m: CommandMode) -> Result<CatFileOptions, CommandantError<InitOptions.ClientError>> {
